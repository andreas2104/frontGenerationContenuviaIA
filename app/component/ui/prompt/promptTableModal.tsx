...
export default function PromptTableModal() {
  ...
  const confirmDelete = (prompt: Prompt) => {
    setPromptToDelete({
      id: prompt.id,
      nom: prompt.nom_prompt || 'ce prompt'
    });
  };
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(promptToDelete.id);
    
    try {
      await deletePrompt(promptToDelete.id);
      showNotification(`🗑️ Prompt "${promptToDelete.nom}" supprimé avec succès !`, 'success');
    } catch (error) {
  ...
  const handleModalSuccess = (type: 'add' | 'edit', nomPrompt: string) => {
    if (type === 'add') {
      showNotification(`✅ Prompt "${nomPrompt}" créé avec succès !`, 'success');
    } else {
      showNotification(`🔄 Prompt "${nomPrompt}" modifié avec succès !`, 'success');
    }
  };
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 animate-scale-in">
            ...
            <div className="mt-2">
              <p className="text-sm text-gray-600">
                Êtes-vous sûr de vouloir supprimer le prompt <strong>"{promptToDelete.nom}"</strong> ?
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-700">{p.date_creation}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">Modifié le:</span>
                        <span className="text-gray-700">{p.date_modification}</span>
                      </div>
                    </div>
                  </div>
